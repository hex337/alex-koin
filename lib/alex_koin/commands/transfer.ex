defmodule AlexKoin.Commands.Transfer do
  ...
  def execute(user, message, slack, memo, to_slack_id, amount, localization) do
    ...
    Logger.info("#{user.first_name} is transfering #{amt} koin to #{to_user.first_name}.",
      ansi_color: :green
    )
  ...
  defp transfer_reply(usr_amount, amt, _user, _to_user, message, _memo, _slack, _loc)
       when usr_amount < amt,
       do: {"Not enough koin to do that transfer.", SlackDataHelpers.message_ts(message)}

  defp transfer_reply(_usr_amount, _amt, user, to_user, message, _memo, _slack, _loc)
       when user == to_user,
       do: {"How about a big ol' ball of nope.", SlackDataHelpers.message_ts(message)}
  ...
  defp transfer_reply_post("us"), do: "."
  defp transfer_reply_post("ca"), do: ", eh?"
