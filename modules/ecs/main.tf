loudwatch Log Group
======*/
resource "aws_cloudwatch_log_group" "alexkoin" {
  name = "alexkoin"

  tags {
    Environment = "${var.environment}"
    Application = "Alex Koin"
  }
}
...
======*/
resource "aws_ecr_repository" "alexkoin_app" {
...
data "template_file" "server_task" {
  template = "${file("${path.module}/tasks/server_task_definition.json")}"

  vars {
    image           = "${aws_ecr_repository.alexkoin_app.repository_url}"
    ...
    log_group       = "${aws_cloudwatch_log_group.alexkoin.name}"
  }
}
...
resource "aws_alb" "alb_alexkoin" {
  name            = "${var.environment}-alb-alexkoin"
...
resource "aws_alb_listener" "alexkoin" {
  load_balancer_arn = "${aws_alb.alb_alexkoin.arn}"
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  alarm_name          = "${var.environment}_alexkoin_web_cpu_utilization_high"
