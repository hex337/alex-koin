defmodule AlexKoin.Test.SlackSendStub do
  ...
  def init do
    :ets.new(@ets, [:set, :named_table, :public, read_concurrency: true, write_concurrency: true])
    {:ok, %{}}
  end
  ...
  def send_raw(message_json, slack) do
    mod_json =
      message_json |> String.replace(~r(`........-....-....-....-............`), "uuid here")
  ...
  defp return!(fun_name, args, matches) do
    case matches do
      ...
      _result ->
        raise RuntimeError,
              "Didn't find a response for #{__MODULE__}:#{fun_name} " <> "with #{inspect(args)}"
    end
  end
end
